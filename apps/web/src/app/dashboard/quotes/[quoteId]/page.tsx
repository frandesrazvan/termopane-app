    ...
    <section id="documents" className="mt-6 rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Immutable Template A PDFs for version {currentVersion?.versionNumber ?? "-"}
          </p>
        </div>
      ...
      {documentEvent ? (
        <p className="mt-4 rounded-md bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-800">
          PDF document was generated and stored for this quote version.
        </p>
      ) : null}
      {documentError ? (
        <p className="mt-4 rounded-md bg-rose-50 px-3 py-2 text-sm font-medium text-rose-800">
          {documentError === "locked"
            ? "Lock this quote version before generating a customer-facing PDF."
            : "The PDF could not be generated. Check the quote version and local storage."}
        </p>
      ) : null}
      ...
      {documents.length > 0 ? (
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs font-medium text-zinc-600">
                    {document.createdAt.toLocaleString("ro-RO")}
                  </p>
                  {document.checksum ? (
        ...
        <p className="mt-4 rounded-md bg-stone-100 p-4 text-sm text-zinc-700">
          No PDFs have been generated for this quote version yet.
        </p>
      )}
    </section>
  );
